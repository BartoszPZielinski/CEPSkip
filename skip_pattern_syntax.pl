...
/*
    Syntax: pattern ::= empty | event(typ, variable) | iter(pattern) |
    filter(pattern, condition)
    | unless(pattern, event(type, variable), condition) |
...
pattern_binds(filter(Q, _), V)
     :- pattern_binds(Q, V).
pattern_binds(unless(Q, _, _), V)
     :- pattern_binds(Q, V).
pattern_binds(Q within _, V)
    :- pattern_binds(Q, V).
...
is_unique_pattern_(filter(P, _))
    --> is_unique_pattern_(P).
is_unique_pattern_(unless(P, E, _))
    --> is_unique_pattern_(P),
...
/*
    Make unique pattern.
    State: first unused var number, renaming from old vars binding vars to new vars.
    The state u(N, Trans)
*/
...
pattern_unique(unless(P0, E0, C0), unless(P, E, C))
    --> pattern_unique(P0, P),
        renaming(A0),
        pattern_unique(E0, E),
        renaming(A),
...
pattern_unique(P1 or P2, U1 or U2)
    --> renaming(A0),
        ...
        set_renaming(_, A).
pattern_unique(P0 within T, P within T)
    --> pattern_unique(P0, P).
